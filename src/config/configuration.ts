import { Config, Developement, Objectype } from './config.interface';
...
export const configuration = async (): Promise<Config> => {
    ...
    const { config: environment } = await import(`./envs/${process.env.NODE_ENV || 'dev'}`);
...
//     const test = util.merge(config, environment);
//     console.log(test);
